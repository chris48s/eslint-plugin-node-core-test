...
function createContextMethodTracker(methodName) {
  // Each entry is the name of the first parameter of a test/suite callback,
  // or null when there is no trackable parameter.
    ...
    if (parent && parent.type === "CallExpression") {
      ...
      if (
        ...
        if (firstParam && firstParam.type === "Identifier") {
          contextParamStack.push(firstParam.name);
          return;
        }
      }
    }
  ...
  function isContextMethodCall(node) {
    ...
    return contextParamStack.some((name) => name === obj.name);
  }
