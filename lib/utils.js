 ...
 * @param {Set<string>} [options.callbackIdentifiers] - Set of function names
 *   that are passed by reference as test/suite callbacks; used to track
 *   `FunctionDeclaration` nodes whose names appear in this set
 ...
 */
function createContextMethodTracker(methodName, options = {}) {
  ...
  const callbackIdentifiers = options.callbackIdentifiers || new Set();
  ...
  function pushContextParam(funcNode) {
    ...
    // Inline callbacks: `describe("x", (t) => { ... })` or
    // `test("x", function (t) { ... })`
    ...
    // Callbacks passed by reference via a FunctionDeclaration:
    //   function handler(t) { ... }
    //   test("x", handler);
    if (
      funcNode.type === "FunctionDeclaration" &&
      funcNode.id &&
      callbackIdentifiers.has(funcNode.id.name)
    ) {
      const firstParam = funcNode.params[0];
      if (firstParam && firstParam.type === "Identifier") {
        contextParamStack.push({ name: firstParam.name, funcNode });
        return;
      }
      contextParamStack.push(null);
      return;
    }
    // Not a tracked test/suite callback.
    contextParamStack.push(null);
  }
